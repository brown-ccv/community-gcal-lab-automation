...
- **Authentication**: Cloud Run edge authentication + service-account Calendar API access
...
1. **Service account Calendar access setup**

  - Ensure your service account JSON is available as a Secret Manager value (`google-admin-sa-json`)
  - The JSON must have a valid `client_email` and `private_key`
  - Share each target calendar with that service account email as **"Make changes to events"**
  - No OAuth consent screen is required for this deployment mode
...
gcloud secrets describe google-admin-sa-json --project=YOUR_PROJECT_ID
```
...
gcloud run deploy gcal-lab-automation \
  ...
  --set-secrets="SESSION_SECRET=session-secret:latest,REMINDER_CALENDAR_ID=reminder-calendar-id:latest,RETENTION_CALENDAR_ID=retention-calendar-id:latest,REQUIRED_GOOGLE_GROUP=required-google-group:latest,GOOGLE_ADMIN_USER_EMAIL=google-admin-user-email:latest,GOOGLE_ADMIN_SERVICE_ACCOUNT_JSON=google-admin-sa-json:latest" \
  --project=${PROJECT_ID} \
  --quiet
```
...
### Step 2.6: Grant Cloud Run Invoker Access

Grant access to the users or group that should open the app.

```bash
gcloud run services add-iam-policy-binding gcal-lab-automation \
  --region=${REGION} \
  --member="group:carelab-group@brown.edu" \
...
2. Sign in with a permitted Brown account if prompted by Google edge auth
...
gcloud secrets delete google-admin-sa-json --project=${PROJECT_ID} --quiet
...
#### 3. Calendar writes fail with 403
...
**Solutions**:
1. Open target calendar settings → Share with specific people
2. Add the service account email from `google-admin-sa-json` (`client_email`)
3. Grant **Make changes to events**
4. Retry event creation

#### 4. "Secret not found" error

**Symptoms**:
```
ERROR: Secret version [google-admin-sa-json:latest] not found
```
...
gcloud secrets get-iam-policy google-admin-sa-json --project=${PROJECT_ID}
```
...
gcloud secrets versions access latest --secret=google-admin-sa-json > /tmp/.env
